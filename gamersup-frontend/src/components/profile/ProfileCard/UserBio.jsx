...
function UserBio({ gamerId, bio }) {
  ...
  return (
    <div
      id='bio'
      className='col-span-2 grid grid-cols-1 bg-base-200 rounded-[16px] shadow-inner shadow-black p-2'
    >
      ...
      {!changing && (
        <h2 className='inline-flex text-neutral-content py-1 text-2xl ml-2'>
          {currentBio}
        </h2>
      )}
      {changing && (
        <form onSubmit={uploadBio}>
          <textarea
            className='inline-flex py-1 text-xl text-black w-full'
            id='bioBlock'
            rows='1'
