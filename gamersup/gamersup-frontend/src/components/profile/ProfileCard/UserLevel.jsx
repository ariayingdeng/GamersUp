...
function UserLevel({ gamerId, level }) {
  ...
  return (
    <div
      className='justify-center flex bg-base-200 rounded-[16px] p-2 grid grid-cols-1 gap-4 shadow-inner shadow-black'
      id='userBirthday'
    >
      <h2 className='inline-flex text-accent-focus py-1 text-xl '>
        ...
        {gamerId === user.id && (
          <button
            className='btn-ghost badge badge-outline text-xs ml-10 my-auto p-3 '
            onClick={levelChange}
          >
            <FaPencilAlt />
          </button>
      ...
      {!changing && (
        <h2 className='inline-flex text-neutral-content py-1 text-3xl'>
          &nbsp;&nbsp;{currentLevel === 0 && <p>Newbie</p>}
          {currentLevel === 1 && <p>Veteran</p>}
          {currentLevel === 2 && <p>Pro</p>}
        </h2>
      )}
      {changing && (
