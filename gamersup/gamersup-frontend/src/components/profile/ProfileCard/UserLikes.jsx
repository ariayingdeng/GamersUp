...
function UserLikes({ gamerId, likes, socket }) {
  const { user, changeLikes, getLikes } = useContext(UserContext);
  const [currentLikes, setCurrentLikes] = useState(likes);
  const params = useParams();

  useEffect(() => {
    getLikes(params.id).then((response) => {
      setCurrentLikes(response.data);
    });
  }, [currentLikes]);

  const likesAdd = async () => {
    await changeLikes(params.id).then((response) => {
  ...
  return (
    <div className='justify-center flex bg-base-200 rounded-[16px] p-2 grid grid-cols-1 gap-4 shadow-inner shadow-black'>
      <h2 className='inline-flex text-accent-focus py-1 text-xl'>
        ...
        {gamerId !== user.id && (
          <button
            className='btn-ghost badge badge-outline text-xs hover:bg-primary-focus mx-4 my-auto p-3'
            onClick={likesAdd}
          >
